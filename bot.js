...
clickbait('../node_server/root/info/theinfostuff/cmds.txt', Object.values(client.commands).map(c => `<div id="${c.label} ${c.aliases[0] ? c.aliases.join(' ') : ''}">${c.label} ${c.usage}<br>${c.fullDescription}<br>Aliases: ${c.aliases[0] ? c.aliases.join(', ') : 'none'}</div>`).join('<br><br>'))
fs.readdir('./good_memes_probably/', (err, files) => {
    clickbait('../node_server/root/info/theinfostuff/memes.txt', files.join(', ').replace(/.meme/g, ''))
});
clickbait('../node_server/root/info/theinfostuff/guilds.txt', client.guilds.size)
